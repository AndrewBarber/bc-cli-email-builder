...
const download = () => {
  ...
  return program
    .addArgument(
      new Argument(
        '[email-template]',
        'Email template to download from store',
      ).default('all'),
    )
    .option(
      '-p, --path <path>',
      'Path to download the email template',
      './templates',
    )
    .option(
      '-o, --overwrite',
      'Overwrite the email template if it already exists',
      false,
    )
    .description(
      'Download the email templates to the store belonging to the env config',
    )
    .action((emailTemplate: string, options) => {
      ...
      if (
        emailTemplate !== 'all' &&
        !EMAIL_TEMPLATE_TYPE_IDS.includes(emailTemplate)
      ) {
